...
def qfix(q):
    ...
    result[1:][mask] *= -1
    return result
def train_processing(input_folder, output_folder):
...
def generate_train_windows_data():
    train_input_folder = os.path.join('data', 'train_sample_data')
    train_output_folder = os.path.join('data', 'training_windows_data')
    ...
    train_processing(train_input_folder, train_output_folder)