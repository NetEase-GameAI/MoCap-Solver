...
def generate_test_data(SEED):
    ...
    test_motion_set = motion_set[1860:]
    ...
    test_person_set_idx = person_idx_set_idx[1530:]
    ...
    for npz_frame_idx in tqdm(test_motion_set[:]):
        ...
        for mc_select_idx in range(3):
            ...
            person_rand_number = np.random.choice(test_person_set_idx)
            N = len(cdata['poses'])
            poses = cdata['poses'][:, :]
            trans = cdata['trans']
            ...
            J_t = ((preR.dot(J_t.reshape(-1, 3).T)).T).reshape(-1, 24, 3)
            for i in range(N):
                for j in range(24):
                    J_R[i, j] = preR.dot(J_R[i, j])

            output_file = os.path.join(out_valid_file, folder_name + '_' + filename + '_' + str(mc_select_idx) + '.npz')
            ...
            for i in range(N):
                ...
                M1[i, non_ref_idx] = corrupt(M[i, non_ref_idx], 0.1, 0.1, 0.1)
